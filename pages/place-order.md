...
| TimeInforce             | Indicates the time frame in which the order will be active. Possible Values: • Day. The order automatically expires at the end of the regular trading session if it weren't executed.• GoodTillCancel (Good-till-Canceled). The order persists indefinitely until it is executed or manually cancelled.• GoodTillDate. The order will be active until the date specified in the ExpireDate attribute (unless it is executed or cancelled). |
...
| ExecutionInstructions   |  Execution instructions of the order. May include the following data: • `"PerTradeCommission": "1"`. Specified in dollars ($1 per trade).• `"PerContractCommission":"1"`. Specified in cents (1 cent per contract).                                                                                                                                                                                                                    |
